...
class MapRenderer:
    ...
    def __init__(self, sprite_manager=None):
        """Запоминает SpriteManager; без него рисует примитивные фигуры"""
    ...
    def _blit_scaled(self, screen, sprite, sx, sy, target_size, width_multiplier=1.0):
        """Масштабирует спрайт под target_size"""
    ...
    def _tower_screen_size(self, camera, cell_size):
        """Диаметр спрайта башни в пикселях"""
    ...
    def _enemy_screen_size(self, camera, type_name=None):
        """Диаметр спрайта врага в пикселях"""
    ...
    def _draw_background(self, screen, camera, width, height):
        """Рисует фон карты под всеми объектами"""
    ...
    def render(self, screen, camera, session, controller, tower_options, width, height):
        ...
        self._draw_operator(screen, camera, getattr(session, "operator", None))
        self._draw_placement_preview(screen, camera, session, controller, tower_options)

    def _draw_operator(self, screen, camera, operator):
        if operator is None or not operator.is_alive():
            return
        import math
        x, y = camera.world_to_screen(operator.position.x, operator.position.y)
        radius = max(6, int(10 * camera.zoom))
        color = (80, 200, 255) if operator.kind == "assault" else (100, 240, 150)
        pygame.draw.circle(screen, (15, 25, 30), (int(x), int(y)), radius + 3)
        pygame.draw.circle(screen, color, (int(x), int(y)), radius)
        angle = math.atan2(operator.aim.y - operator.position.y, operator.aim.x - operator.position.x)
        pygame.draw.line(screen, (240, 245, 250), (x, y),
                         (x + math.cos(angle) * radius * 2, y + math.sin(angle) * radius * 2), 3)
        pygame.draw.rect(screen, (90, 35, 35), (x - 18, y - radius - 12, 36, 4))
        pygame.draw.rect(screen, color, (x - 18, y - radius - 12,
                                        36 * operator.health / operator.max_health, 4))
        if operator.manual:
            pygame.draw.circle(screen, (255, 225, 100), (int(x), int(y)), radius + 6, 1)

    def _draw_sector_overlay(self, screen, camera, session, width, height):
        """Затемняет ещё не открытые секторы карты"""
    ...
    def _draw_pending_landings(self, screen, camera, session):
        """Рисует маркеры высадки корпоратов"""
        threat_strategies = getattr(session, "threat_strategies", {}) or {}
    ...
    def _draw_base_power_radius(self, screen, camera, session, sx, sy):
        """Рисует радиус стартового питания от базы"""
    ...
    def _draw_power_links(self, screen, camera, session):
        """Рисует линии энергосети между запитанными узлами"""
