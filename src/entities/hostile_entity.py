...
class HostileEntity(Entity, ABC):
    ...
    def breaks_through(self) -> bool:
        """Проверяет, игнорирует ли враг патрулирование на границе известной башни
        (см. Map._advance_towards_base) и отступление на лечение (Map.update) - он
        просто прёт напролом к цели, не выжидая брешь в обороне и не отступая, пока
        жив (см. BioTitan). По умолчанию False - патрулирование и отступление
        остаются обычным поведением большинства врагов."""
        return False
